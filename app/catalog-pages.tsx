...
function CatalogTable({ kind, locale, items, role, onOpen, onEdit, onDelete }: { kind: CatalogKind; locale: Locale; items: CatalogItem[]; role: Role; onOpen: (item: CatalogItem) => void; onEdit: (item: CatalogItem) => void; onDelete: (item: CatalogItem) => void }) {
  const l = labels[locale];
  const isClickableKind = ["carburetor", "mechanic", "driver", "team", "vehicle"].includes(kind);
  return <div className="table-wrap"><table className="results zebra catalog-table"><thead><tr>{headers(kind, locale).map((header) => <th key={header}>{header}</th>)}{role !== "mechanic" && <th>{l.actions}</th>}</tr></thead><tbody>{items.map((item) => <tr key={item.id} className={`${isClickableKind ? "clickable-row" : ""}${kind === "driver" && !(item as DriverRecord).isActive ? " inactive-record" : ""}`} onClick={() => onOpen(item)} tabIndex={isClickableKind ? 0 : undefined} onKeyDown={(event) => { if (isClickableKind && (event.key === "Enter" || event.key === " ")) onOpen(item); }}>{cells(kind, item, locale).map((cell, index) => <td key={index}>{cell}</td>)}{role !== "mechanic" && <td onClick={(event) => event.stopPropagation()}><div className="record-actions">{(kind === "driver" || kind === "team") && <button className="card-action" type="button" onClick={() => onOpen(item)}>{locale === "cs" ? "Karta" : "Card"}</button>}<button type="button" onClick={() => onEdit(item)}>{l.edit}</button>{role === "superadmin" && <button className="delete" type="button" onClick={() => onDelete(item)}>{l.delete}</button>}</div></td>}</tr>)}</tbody></table></div>;
}
...
function CatalogForm({ kind, locale, item, teams, carburetorTypes, onClose, onSaved }: { kind: CatalogKind; locale: Locale; item: CatalogItem | null; teams: TeamRecord[]; carburetorTypes: CarburetorTypeRecord[]; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal" role="dialog" aria-modal="true" aria-labelledby="catalog-form-title" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">MM DIRECTORY</span><h2 id="catalog-form-title">{editing ? `${l.edit} · ${l[kind][1]}` : `${l.new} · ${l[kind][1]}`}</h2></div><button className="close-button" type="button" onClick={onClose} aria-label={locale === "cs" ? "Zavřít" : "Close"}>×</button></div><form onSubmit={submit}><div className="form-grid"><CatalogFields kind={kind} locale={locale} item={item} teams={teams} carburetorTypes={carburetorTypes} /></div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><span className="modal-actions-spacer" /><button className="secondary-compact" type="button" onClick={onClose}>{l.cancel}</button><button className="primary-button" type="submit" disabled={saving}>{saving ? l.saving : l.save}</button></div></form></section></div>;
}
...
function CarburetorTypeForm({ locale, item, onClose, onSaved }: { locale: Locale; item: CarburetorTypeRecord | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal" role="dialog" aria-modal="true" aria-labelledby="carb-type-form-title" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">MASTER DATA</span><h2 id="carb-type-form-title">{item ? (locale === "cs" ? "Upravit typ karburátoru" : "Edit carburetor type") : (locale === "cs" ? "Nový typ karburátoru" : "New carburetor type")}</h2></div><button className="close-button" type="button" onClick={onClose} aria-label={locale === "cs" ? "Zavřít" : "Close"}>×</button></div><form onSubmit={submit}><div className="form-grid">
  ...
  return <section className="dash-panel data-panel latest-carb-panel">
    ...
    {pageItems.length === 0 ? <EmptyState size="compact" variant="filtered" title={locale === "cs" ? "Žádný karburátor neodpovídá filtru." : "No carburetor matches the filter."} /> : <div className="table-wrap"><table className="results zebra latest-carb-table">
      ...
      <tbody>{pageItems.map((item, index) => {
        ...
        return <tr key={item.id} className="clickable-row" onClick={() => onOpen(item)} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(item); }}>
