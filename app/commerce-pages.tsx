...
export type CustomerRecord = {
  ...
  saleCount?: number; createdAt: number; updatedAt: number;
};
...
function CommerceForm({ kind, locale, item, onClose, onSaved }: { kind: PageKind; locale: Locale; item: CommerceRecord | null; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal commerce-modal" role="dialog" aria-modal="true" aria-labelledby="commerce-form-title" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">{config.eyebrow}</span><h2 id="commerce-form-title">{item ? (locale === "cs" ? "Upravit záznam" : "Edit record") : (locale === "cs" ? "Nový záznam" : "New record")}</h2></div><button className="close-button" type="button" onClick={onClose} aria-label={locale === "cs" ? "Zavřít" : "Close"}>×</button></div><form onSubmit={submit}><div className="form-grid">{kind === "customers" ? <CustomerFields item={item as CustomerRecord | null} locale={locale} /> : kind === "services" ? <ServiceFields item={item as ServiceCatalogRecord | null} locale={locale} /> : <PartFields item={inventoryItem} locale={locale} categories={partCategories} onCategories={setPartCategories} imagePreview={partImagePreview} onImage={(file) => { setPartImage(file); setRemovePartImage(false); if (file) setPartImagePreview(URL.createObjectURL(file)); }} onRemoveImage={() => { setPartImage(null); setPartImagePreview(""); setRemovePartImage(Boolean(inventoryItem?.imageUrl)); }} />}</div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><span className="modal-actions-spacer" /><button className="secondary-compact" type="button" onClick={onClose}>{locale === "cs" ? "Zrušit" : "Cancel"}</button><button className="primary-button" type="submit" disabled={saving}>{saving ? (locale === "cs" ? "Ukládám…" : "Saving…") : (locale === "cs" ? "Uložit" : "Save")}</button></div></form></section></div>;
}

function CustomerFields({ item, locale }: { item: CustomerRecord | null; locale: Locale }) { return <><label><span>{locale === "cs" ? "Jméno / firma" : "Name / company"} *</span><input name="name" defaultValue={item?.name ?? ""} required autoFocus /></label><label><span>{locale === "cs" ? "Telefon" : "Phone"}</span><input name="phone" defaultValue={item?.phone ?? ""} /></label><label><span>E-mail</span><input name="email" type="email" defaultValue={item?.email ?? ""} /></label>
  <label><span>{locale === "cs" ? "Země" : "Country"}</span><CountrySelect name="countryCode" defaultValue={item?.countryCode ?? ""} locale={locale} /></label>
  <label><span>IČO</span><input name="companyId" defaultValue={item?.companyId ?? ""} /></label><label><span>DIČ</span><input name="vatId" defaultValue={item?.vatId ?? ""} /></label><label className="full-field"><span>{locale === "cs" ? "Adresa" : "Address"}</span><input name="address" defaultValue={item?.address ?? ""} /></label>
  {/* Výchozí slevy pro zakázkový servis — na zakázce se dají přepsat, u jednotlivé položky taky. */}
  <label><span>{locale === "cs" ? "Výchozí sleva na práci" : "Default work discount"} (%)</span><input name="discountWorkPercent" type="number" min="0" max="100" step="1" defaultValue={item?.discountWorkPercent ?? 0} /></label>
  <label><span>{locale === "cs" ? "Výchozí sleva na materiál" : "Default material discount"} (%)</span><input name="discountMaterialPercent" type="number" min="0" max="100" step="1" defaultValue={item?.discountMaterialPercent ?? 0} /></label>
  <label className="full-field"><span>{locale === "cs" ? "Poznámka" : "Notes"}</span><textarea name="notes" rows={3} defaultValue={item?.notes ?? ""} /></label></>; }
