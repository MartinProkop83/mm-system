...
export function CircuitsPage({ locale, role }: { locale: Locale; role: Role }) {
  ...
  async function load(){setLoading(true);try{const response=await fetch("/api/circuits",{cache:"no-store"});if(!response.ok)throw new Error();setCircuits(((await response.json()) as {circuits:CircuitRecord[]}).circuits);}finally{setLoading(false);}}
  useEffect(()=>{void load();},[]);
