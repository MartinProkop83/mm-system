...
export function CircuitsPage({ locale, role }: { locale: Locale; role: Role }) {
  const [circuits,setCircuits]=useState<CircuitRecord[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(false); const [form,setForm]=useState<CircuitRecord|null|undefined>(undefined); const [country,setCountry]=useState("");
  const canManage=role!=="mechanic";
  async function load(){setLoading(true);try{const response=await fetch("/api/circuits",{cache:"no-store"});if(!response.ok)throw new Error();setCircuits(((await response.json()) as {circuits:CircuitRecord[]}).circuits);setError(false);}catch{setError(true);}finally{setLoading(false);}}
  useEffect(()=>{void load();},[]);
  ...
  {loading?<section className="panel"><LoadingState /></section>:error?<section className="panel"><EmptyState variant="error" icon="!" title={locale==="cs"?"Tratě se nepodařilo načíst.":"Could not load circuits."} action={<button className="secondary-compact" type="button" onClick={()=>{void load();}}>{locale==="cs"?"Zkusit znovu":"Try again"}</button>} /></section>:groups.length===0?<section className="panel"><EmptyState title={locale==="cs"?"Zatím žádné tratě":"No circuits yet"} /></section>:groups.map((group)=><section className="circuit-country-group" key={group.code}><header><h2>{countryFlag(group.code)} {countries.find((item)=>item.code===group.code)?.name??group.code}</h2><span>{group.items.length}</span></header><div className="circuit-card-grid">{group.items.map((item)=><article className="panel circuit-card" key={item.id}><div className="circuit-card-image">{item.imageUrl?<img src={item.imageUrl} alt={item.name}/>:<span>⌁</span>}</div><div className="circuit-card-body"><span className="eyebrow">{countryFlag(item.countryCode)} {item.countryCode}</span><h3>{item.name}</h3><p>{item.address||"—"}</p><div className={item.latitude!==null&&item.longitude!==null?"circuit-location-state ready":"circuit-location-state missing"}>{item.latitude!==null&&item.longitude!==null?(locale==="cs"?"✓ Poloha a počasí připravené":"✓ Location and weather ready"):(locale==="cs"?"! Doplň polohu pro počasí":"! Add location for weather")}</div><div className="circuit-trip">{item.distanceKm!==null&&<strong>{formatNumber(item.distanceKm)} km</strong>}{item.driveMinutes!==null&&<span>≈ {formatMinutes(item.driveMinutes,locale)}</span>}</div><div className="circuit-links">{item.mapsUrl&&<a href={item.mapsUrl} target="_blank" rel="noreferrer">⌖ Google Maps</a>}{item.websiteUrl&&<a href={item.websiteUrl} target="_blank" rel="noreferrer">↗ Web</a>}</div></div>{canManage&&<footer><button className="secondary-compact" type="button" onClick={()=>setForm(item)}>{locale==="cs"?"Upravit":"Edit"}</button>{role==="superadmin"&&<button className="danger-compact" type="button" onClick={()=>void archive(item)}>{locale==="cs"?"Smazat":"Delete"}</button>}</footer>}</article>)}</div></section>)}
...
function CircuitForm({locale,circuit,onClose,onSaved}:{locale:Locale;circuit:CircuitRecord|null;onClose:()=>void;onSaved:()=>void}){
  ...
  return <div className="modal-backdrop" onMouseDown={(event)=>{if(event.target===event.currentTarget)onClose();}}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal circuit-modal" role="dialog" aria-modal="true" aria-labelledby="circuit-form-title" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">MM CIRCUIT DIRECTORY</span><h2 id="circuit-form-title">{circuit?(locale==="cs"?"Upravit trať":"Edit circuit"):(locale==="cs"?"Nová trať":"New circuit")}</h2></div><button className="close-button" type="button" onClick={onClose} aria-label={locale==="cs"?"Zavřít":"Close"}>×</button></div><form onSubmit={submit}><div className="form-grid">
    ...
    <label className="full-field circuit-image-field"><span>{locale==="cs"?"Obrázek / mapa tratě":"Circuit image / map"}</span><input name="image" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event)=>{const file=event.target.files?.[0];if(file)setPreview(URL.createObjectURL(file));}}/>{preview&&<img src={preview} alt="Preview"/>}<small>{locale==="cs"?"PNG, JPG nebo WebP; libovolný poměr stran, maximálně 10 MB.":"PNG, JPG or WebP; any aspect ratio, max 10 MB."}</small></label>
