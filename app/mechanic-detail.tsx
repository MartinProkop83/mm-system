...
function MechanicRaceTravel({ assignment, locale, compact = false }: { assignment: RaceAssignment; locale: Locale; compact?: boolean }) {
  ...
  return <section className={`mechanic-race-travel ${compact ? "compact" : ""}`}><header><div><span>MM TRAVEL</span><strong>{assignment.raceName} · {assignment.track}</strong></div><b>{total ? formatCount(total, locale, RECORD_FORMS) : (locale === "cs" ? "Nezadáno" : "Not entered")}</b></header><div className="mechanic-travel-grid">
