...
export function CalendarPage({ locale, onOpenRace }: { locale: Locale; onOpenRace: (raceId: string) => void }) {
  ...
  return <div className="calendar-page">
    <section className="panel calendar-header">
    ...
    <section className="panel calendar-board">
      <header><button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}>‹</button><div><h3>{monthLabel}</h3><button type="button" onClick={() => { const today = new Date(); setMonth(new Date(today.getFullYear(), today.getMonth(), 1)); }}>{locale === "cs" ? "Dnes" : "Today"}</button></div><button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}>›</button></header>
    ...
    <section className="calendar-agenda"><div className="calendar-agenda-heading"><h3>{locale === "cs" ? "Přehled výjezdů" : "Travel overview"}</h3><span>{races.length} {locale === "cs" ? "závodů" : "races"}</span></div>{races.length === 0 ? <div className="panel empty-state"><h3>{locale === "cs" ? "Pro tento filtr nejsou žádné výjezdy" : "No trips match this filter"}</h3><p>{locale === "cs" ? "Volné dny zůstávají v kalendáři bez označení." : "Free days remain blank in the calendar."}</p></div> : races.map((race) => <RaceAgenda key={race.id} race={race} data={data} locale={locale} />)}</section>
  </div>;
}
