...
export function RaceLogisticsPanel({ raceId, locale, role }: { raceId: string; locale: Locale; role: Role }) {
  ...
  const canManage = role !== "mechanic";
  const load = useCallback(async () => { const response = await fetch(`/api/logistics?raceId=${encodeURIComponent(raceId)}`, { cache: "no-store" }); if (response.ok) setData((await response.json()) as LogisticsData); }, [raceId]);
...
function RaceTravelGroup({ kind, records, locale, onOpen }: { kind: LogisticsKind; records: TravelRecord[]; locale: Locale; onOpen: (record: TravelRecord) => void }) {
  return <section className={`race-travel-group kind-${kind}`}><header><span>{kindIcon(kind)}</span><strong>{kindTitle(kind, locale)}</strong><b>{records.length}</b></header>{records.length ? <div>{records.map((record) => { const summary = recordSummary(kind, record, locale); const accommodation = kind === "accommodation" ? record as AccommodationRecord : null; return <button className="race-travel-entry" type="button" key={record.id} onClick={() => onOpen(record)}><span><b>{summary.title}</b><small>{accommodation?.address || summary.place}</small>{accommodation && (accommodation.trackDistanceKm !== null || accommodation.trackDriveMinutes !== null) && <div className="race-accommodation-trip"><span>{locale === "cs" ? "NA TRAŤ" : "TO CIRCUIT"}</span>{accommodation.trackDriveMinutes !== null && <strong>≈ {formatDriveMinutes(accommodation.trackDriveMinutes)}</strong>}{accommodation.trackDistanceKm !== null && <small>{formatDecimal(accommodation.trackDistanceKm, locale)} km</small>}</div>}{kind === "flight" && <em><b>{locale === "cs" ? "Letí:" : "Passengers:"}</b> {passengerNames(record as FlightRecord, locale)}</em>}</span><strong>{summary.time}</strong><em className={record.attachments.length ? "has-files" : ""}>📎 {record.attachments.length}</em><i>›</i></button>; })}</div> : <p>{locale === "cs" ? "Zatím nezadané" : "Not entered"}</p>}</section>;
}
...
function formatDriveMinutes(value: number) { const hours = Math.floor(value / 60); const minutes = Math.round(value % 60); if (!hours) return `${minutes} min`; return minutes ? `${hours} h ${minutes} min` : `${hours} h`; }
function formatAccommodationRoute(item: AccommodationRecord, locale: Locale) { const parts: string[] = []; if (item.trackDistanceKm !== null) parts.push(`${formatDecimal(item.trackDistanceKm, locale)} km`); if (item.trackDriveMinutes !== null) parts.push(`≈ ${formatDriveMinutes(item.trackDriveMinutes)}`); return parts.join(" · "); }
