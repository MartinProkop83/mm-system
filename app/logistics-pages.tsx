...
function LogisticsForm({ kind, locale, races, travelers, record, lockedRaceId, onClose, onSaved }: { kind: LogisticsKind; locale: Locale; races: LogisticsRace[]; travelers: TravelPassenger[]; record: TravelRecord | null; lockedRaceId?: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal logistics-modal" role="dialog" aria-modal="true" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">MM TRAVEL</span><h2>{recordId ? (locale === "cs" ? "Upravit" : "Edit") : (locale === "cs" ? "Nový záznam" : "New record")} · {kindTitle(kind, locale).toLocaleLowerCase(locale === "cs" ? "cs" : "en")}</h2></div><button className="close-button" type="button" onClick={onClose}>×</button></div><form onSubmit={submit}><div className="form-grid">
