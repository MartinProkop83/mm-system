...
export function LogisticsPage({ kind, locale, role }: { kind: LogisticsKind; locale: Locale; role: Role }) {
  ...
  return <div className="logistics-page">
    ...
    <section className="dash-panel logistics-card-panel">
      {loading ? <LoadingState label={locale === "cs" ? "Načítám…" : "Loading…"} /> : error ? <EmptyState variant="error" icon="!" title={locale === "cs" ? "Záznamy se nepodařilo načíst." : "Could not load records."} action={<button className="secondary-compact" type="button" onClick={() => { void load(); }}>{locale === "cs" ? "Zkusit znovu" : "Try again"}</button>} /> : records.length === 0 ? <EmptyState icon={kindIcon(kind)} title={locale === "cs" ? "Zatím bez záznamů" : "No records yet"} description={locale === "cs" ? "První záznam můžeš vytvořit tady nebo přímo v detailu závodu." : "Create the first record here or directly in a race."} /> : <div className="logistics-card-grid">{records.map((record) => <TravelRecordCard key={record.id} kind={kind} record={record} locale={locale} role={role} onOpen={() => setDetail(record)} onEdit={() => setForm(record)} onDelete={() => { void remove(record); }} />)}</div>}
    </section>
...
function TravelDetailModal({ kind, record, locale, canManage, onEdit, onClose }: { kind: LogisticsKind; record: TravelRecord; locale: Locale; canManage: boolean; onEdit: () => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef as React.RefObject<HTMLElement>} className="modal travel-detail-modal" role="dialog" aria-modal="true" aria-labelledby="travel-detail-title" tabIndex={-1}><div className="modal-header"><div><span className="eyebrow">MM TRAVEL · {kindTitle(kind, locale).toLocaleUpperCase(locale === "cs" ? "cs" : "en")}</span><h2 id="travel-detail-title">{recordSummary(kind, record, locale).title}</h2><p>{countryFlag(record.countryCode)} {record.raceName} · {record.raceTrack}</p></div><button className="close-button" type="button" onClick={onClose} aria-label={locale === "cs" ? "Zavřít" : "Close"}>×</button></div><div className="travel-detail-grid">{fields.map(([label, value]) => <div key={label}><small>{label}</small><strong>{value || "—"}</strong></div>)}</div>{accommodation && <AccommodationLinks item={accommodation} locale={locale} />}{flight && <section className="travel-detail-passengers"><h3>{locale === "cs" ? "Cestující" : "Passengers"}</h3>{flight.passengers.length ? <div>{flight.passengers.map((passenger) => <span key={passenger.id}>✓ {passenger.name}</span>)}</div> : <EmptyState size="compact" title={locale === "cs" ? "Bez vybraných členů týmu" : "No team members selected"} />}{flight.passengersNote && <p>{flight.passengersNote}</p>}</section>}{record.notes && <section className="travel-detail-notes"><small>{locale === "cs" ? "Poznámka" : "Notes"}</small><p>{record.notes}</p></section>}<section className="travel-detail-files"><h3>{locale === "cs" ? "Přílohy a náhledy" : "Attachments and previews"}</h3><AttachmentSections attachments={record.attachments} direction={flight?.direction} locale={locale} /></section><div className="modal-actions"><span className="modal-actions-spacer" />{canManage && <button className="secondary-compact" type="button" onClick={onEdit}>{locale === "cs" ? "Upravit záznam" : "Edit record"}</button>}<button className="primary-button" type="button" onClick={onClose}>{locale === "cs" ? "Zavřít" : "Close"}</button></div></section></div>;
}
...
function AttachmentGallery({ attachments, locale, canDelete = false, onDelete }: { attachments: TravelAttachment[]; locale: Locale; canDelete?: boolean; onDelete?: (attachment: TravelAttachment) => void }) {
  if (!attachments.length) return <EmptyState size="compact" title={locale === "cs" ? "Zatím bez příloh" : "No attachments yet"} />;
