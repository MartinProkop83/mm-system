...
export async function GET(request: Request) {
  ...
  const type = await getD1().prepare(`
    SELECT photo_key AS photoKey, photo_content_type AS photoContentType
    FROM carburetor_types WHERE id = ? AND archived_at IS NULL
  `).bind(id).first<{ photoKey: string | null; photoContentType: string | null }>();
  if (!type?.photoKey) return Response.json({ error: "Photo not found" }, { status: 404 });

  const object = await getAssetsBucket().get(type.photoKey);
  if (!object) return Response.json({ error: "Photo not found" }, { status: 404 });
  const headers = new Headers();
  object.writeHttpMetadata(headers);
  headers.set("content-type", type.photoContentType || object.httpMetadata?.contentType || "application/octet-stream");
...
export async function POST(request: Request) {
  ...
  if (!typeId) return Response.json({ error: "Carburetor type id is required" }, { status: 400 });
  ...
  const existing = await d1.prepare(`
    SELECT id, photo_key AS photoKey FROM carburetor_types
    WHERE id = ? AND archived_at IS NULL
  `).bind(typeId).first<{ id: string; photoKey: string | null }>();
  if (!existing) return Response.json({ error: "Carburetor type not found" }, { status: 404 });

  const key = `carburetor-type-photos/${typeId}/${crypto.randomUUID()}.${extension}`;
  const now = Date.now();
  const bucket = getAssetsBucket();
  await bucket.put(key, photo.stream(), {
    httpMetadata: { contentType },
    customMetadata: { typeId, uploadedBy: user.email },
  });
  try {
    await d1.batch([
      d1.prepare(`
        UPDATE carburetor_types
        SET photo_key = ?, photo_content_type = ?, photo_updated_at = ?, updated_at = ?
        WHERE id = ? AND archived_at IS NULL
      `).bind(key, contentType, now, now, typeId),
      d1.prepare(`
        INSERT INTO audit_logs (id, actor_email, action, entity_type, entity_id, details, created_at)
        VALUES (?, ?, 'upload_photo', 'carburetorType', ?, ?, ?)
      `).bind(crypto.randomUUID(), user.email, typeId, JSON.stringify({ contentType, size: photo.size }), now),
    ]);
  } catch (error) {
  ...
  return Response.json({ typeId, updatedAt: now });
}
...
export async function DELETE(request: Request) {
  ...
  const d1 = getD1();
  const existing = await d1.prepare(`
    SELECT photo_key AS photoKey FROM carburetor_types WHERE id = ? AND archived_at IS NULL
  `).bind(typeId).first<{ photoKey: string | null }>();
  if (!existing) return Response.json({ error: "Carburetor type not found" }, { status: 404 });
  const now = Date.now();
  await d1.batch([
    d1.prepare(`UPDATE carburetor_types SET photo_key = NULL, photo_content_type = NULL, photo_updated_at = NULL, updated_at = ? WHERE id = ?`).bind(now, typeId),
    d1.prepare(`INSERT INTO audit_logs (id, actor_email, action, entity_type, entity_id, details, created_at) VALUES (?, ?, 'delete_photo', 'carburetorType', ?, '{}', ?)`).bind(crypto.randomUUID(), user.email, typeId, now),
  ]);
