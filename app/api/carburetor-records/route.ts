...
export async function GET(request: Request) {
  ...
  const d1 = getD1();
  const carburetor = await d1.prepare(`SELECT id, code, carburetor_type_id AS carburetorTypeId, category, family, brand, model, status, notes, sold_at AS soldAt, created_at AS createdAt, updated_at AS updatedAt FROM carburetors WHERE id = ? AND archived_at IS NULL`).bind(id).first<Record<string, unknown>>();
  if (!carburetor) return Response.json({ error: "Carburetor not found" }, { status: 404 });
  const [assignments, services, mechanics, type] = await Promise.all([
    d1.prepare(`
    ...
    carburetor.carburetorTypeId
      ? d1.prepare(`SELECT photo_key AS photoKey, photo_updated_at AS photoUpdatedAt FROM carburetor_types WHERE id = ?`).bind(carburetor.carburetorTypeId).first<{ photoKey: string | null; photoUpdatedAt: number | null }>()
      : Promise.resolve(null),
  ]);
  return Response.json({
    carburetor: { ...carburetor, photoUrl: carburetorTypePhotoUrl(carburetor.carburetorTypeId, type?.photoKey, type?.photoUpdatedAt) },
...
export async function PUT(request: Request) {
  ...
  const serviceDate = clean(payload.serviceDate, 10);
  const serviceType = clean(payload.serviceType, 20);
  if (!id || !carburetorId || !/^\d{4}-\d{2}-\d{2}$/.test(serviceDate) || !serviceTypes.has(serviceType)) return Response.json({ error: "Service date and type are required" }, { status: 400 });
  const d1 = getD1();
  const existing = await d1.prepare("SELECT id FROM carburetor_service_entries WHERE id = ? AND carburetor_id = ?").bind(id, carburetorId).first();
  ...
  const details = { serviceDate, serviceType, mechanicId: mechanic?.id ?? null, mechanicName: mechanic?.name ?? user.fullName, workDone: clean(payload.workDone, 1500), replacedParts: clean(payload.replacedParts, 1000), notes: clean(payload.notes, 1500) };
  await d1.batch([
    d1.prepare("UPDATE carburetor_service_entries SET service_date = ?, service_type = ?, mechanic_id = ?, mechanic_name_snapshot = ?, work_done = ?, replaced_parts = ?, notes = ? WHERE id = ? AND carburetor_id = ?").bind(details.serviceDate, details.serviceType, details.mechanicId, details.mechanicName, details.workDone, details.replacedParts, details.notes, id, carburetorId),
    d1.prepare("INSERT INTO audit_logs (id, actor_email, action, entity_type, entity_id, details, created_at) VALUES (?, ?, 'update_service', 'carburetor', ?, ?, ?)").bind(crypto.randomUUID(), user.email, carburetorId, JSON.stringify(details), now),
  ]);
