...
export function CompetitionHistoryDetail({ entityType, entityId, locale, role, onBack, onEdit }: { entityType: EntityType; entityId: string; locale: Locale; role: Role; onBack: () => void; onEdit: (subject: Subject) => void }) {
  ...
  if (error) return <section className="dash-panel empty-state error-state"><b>!</b><p>{locale === "cs" ? "Historii se nepodařilo načíst." : "Could not load the history."}</p><button className="secondary-compact" type="button" onClick={onBack}>{locale === "cs" ? "Zpět" : "Back"}</button></section>;
  if (!data) return <section className="dash-panel empty-state"><span className="spinner" /><p>{locale === "cs" ? "Načítám kompletní historii…" : "Loading complete history…"}</p></section>;
