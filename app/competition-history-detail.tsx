...
export function CompetitionHistoryDetail({ entityType, entityId, locale, role, onBack, onEdit }: { entityType: EntityType; entityId: string; locale: Locale; role: Role; onBack: () => void; onEdit: (subject: Subject) => void }) {
  ...
  if (error) return <section className="dash-panel"><EmptyState variant="error" icon="!" title={locale === "cs" ? "Historii se nepodařilo načíst." : "Could not load the history."} action={<button className="secondary-compact" type="button" onClick={onBack}>{locale === "cs" ? "Zpět" : "Back"}</button>} /></section>;
  if (!data) return <section className="dash-panel"><LoadingState label={locale === "cs" ? "Načítám kompletní historii…" : "Loading complete history…"} /></section>;
  ...
  return <div className="competition-detail competition-print-area">
    ...
    <section className={`dash-panel competition-detail-hero ${isDriver ? "driver" : "team"}`}>
      <RaceLogoBadge logoUrl={isDriver ? driver?.photoUrl : team?.logoUrl} name={subject.name} fallback={initials || "MM"} size="large" />
    ...
    <section className="mechanic-stat-grid competition-stat-grid">
      <article className="dash-panel"><span>{locale === "cs" ? "Závodů celkem" : "Total races"}</span><strong>{races}</strong></article>
      <article className="dash-panel"><span>{isDriver ? (locale === "cs" ? "Kategorií" : "Categories") : (locale === "cs" ? "Pilotů" : "Drivers")}</span><strong>{isDriver ? categories : drivers}</strong></article>
      <article className="dash-panel"><span>{locale === "cs" ? "Použitých motorů" : "Engines used"}</span><strong>{engines}</strong></article>
      <article className="dash-panel"><span>{locale === "cs" ? "Použitých karburátorů" : "Carburetors used"}</span><strong>{carbs}</strong></article>
    </section>
    ...
    <section className="dash-panel mechanic-history-panel competition-history-panel">
