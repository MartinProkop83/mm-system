...
export function VehicleDetail({ vehicleId, locale, role, onBack, onEdit }: { vehicleId: string; locale: Locale; role: Role; onBack: () => void; onEdit: (vehicle: VehicleRecord) => void }) {
  ...
  if (error) return <section className="dash-panel"><EmptyState variant="error" icon="!" title={locale === "cs" ? "Kartu auta se nepodařilo načíst." : "Could not load the vehicle card."} action={<button className="secondary-compact" type="button" onClick={() => { void load(); }}>{locale === "cs" ? "Zkusit znovu" : "Try again"}</button>} /></section>;
  if (!data) return <section className="dash-panel"><LoadingState label={locale === "cs" ? "Načítám kartu auta…" : "Loading vehicle card…"} /></section>;
